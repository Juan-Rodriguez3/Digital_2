...
void LCD_Sprite(int x, int y, int width, int height, const uint16_t *bitmap,
	...
	// Cierre correcto de LCD_Sprite
	LCD_CS_H();
}
/*
// SetWindows sin manejo de CS para uso interno
static void SetWindows_NoCS(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2) {
    uint8_t cmd;
    uint8_t data;
    ...
    // Datos columna
    LCD_DC_H();
    data = x1 >> 8; HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = x1;      HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = x2 >> 8; HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = x2;      HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    ...
    // Datos fila
    LCD_DC_H();
    data = y1 >> 8; HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = y1;      HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = y2 >> 8; HAL_SPI_Transmit(&hspi1, &data, 1, 1);
    data = y2;      HAL_SPI_Transmit(&hspi1, &data, 1, 1);
...
void LCD_BitmapFromFile(FIL *f, SPI_HandleTypeDef *hspi) {
    ...
    // Leer TODOS los datos de la SD primero, SD activa
    ...
    uint16_t frameBuffer[320 * 240];  // ~150KB — asegúrate que tu RAM lo soporte
    UINT totalRead = 0;
    f_read(f, frameBuffer, sizeof(frameBuffer), &totalRead);

    // SD off antes de tocar LCD
    HAL_GPIO_WritePin(SD_SS_GPIO_Port, SD_SS_Pin, GPIO_PIN_SET);

    // Cambiar a SPI rápido para LCD
    hspi->Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_2;
    HAL_SPI_Init(hspi);

    if (totalRead != 320 * 240 * 2) {
        char dbg[60];
        sprintf(dbg, "SD read error: %lu bytes\r\n", (unsigned long)totalRead);
        HAL_UART_Transmit(&huart2, (uint8_t*)dbg, strlen(dbg), 100);
        return;
    }

    // Ahora dibujar todo desde RAM
    SetWindows(0, 0, 319, 239);
    LCD_CS_L();
    LCD_DC_H();

    for (unsigned int i = 0; i < 320 * 240; i++) {
        uint8_t hi = frameBuffer[i] >> 8;
        uint8_t lo = frameBuffer[i] & 0xFF;
        HAL_SPI_Transmit(hspi, &hi, 1, 1);
        HAL_SPI_Transmit(hspi, &lo, 1, 1);
    }
